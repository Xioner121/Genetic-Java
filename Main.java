...
//-----------------------------------------------------------------------------------------------//
//static long seed = 271376456;//Random # gen. seed
    //static final Random randomNum = new Random(seed);
...
// ---- Termination Conditions ---- //
    static int period = 200;// (500)# of generations before a stagnancy check
...
//static int cluster = 50 j0;//range for cluster termination (added to least possible fitsum)
//-----------------------------------------------------------------------------------------------//
...
public void run() {
    ArrayList<int[]> generation = new ArrayList<int[]>();//current generation
    ArrayList<int[]> elites = new ArrayList<int[]>();//elites to add to new generation
    ArrayList<int[]> matingPool = new ArrayList<int[]>();//selected generation
    int[] allFit = new int[popSize];//respective fitness of generation
    float fitSum = 0;//Sum of all fitness
    int min = 0;//minimum fit value
        int pastfit = 0;
        int change = 0;
...
//Evaluation - determining 'fitness' of each route ---------------------------------------------------------------------------------------------
                ...
                change = pastfit-min;
                    if (Math.abs(change) < difference) {
                        //Start new cycle
                            ...
                            restart = true;
                            cycleTracker++;
                            //System.out.println(" New cycle: " + cycleTracker);
                    } 
                pastfit = min;
    }
    ...
    //Selection - determining mating pool (roulette wheel for now) - implement SUS ----------------------------------------------------------------
        ...
        for (int i = 0; i<select; i++) {//for # of selections for mating pool (mating pool size)...
            ...
            for (int y = 0; y<roulette.size(); y++) {//looping through roulette (index corres. to main population)
                ...
                if (y == 0) {//setting min to 0 to not exceed array
                    ...
                    min2 = roulette.get(y-1);
                }
                if (rand < roulette.get(y) && rand > min2) {//if less than next and greater than current probabilty range...
    ...
    //int randomSpace = Length-sublength;
        for (int i = 0; i < crosses; i++) {
            ...
            //}
            int rand2;
            int rand = (int)(ThreadLocalRandom.current().nextDouble() * (matingPool.size()));
            do {
                rand2 = (int)(ThreadLocalRandom.current().nextDouble() * (matingPool.size()));
            } while (rand == rand2);
            ...
            System.out.println( );
            */
                int sublength = (int)(ThreadLocalRandom.current().nextDouble() * (subMax)) + subMin;
                int subIndex = (int)(ThreadLocalRandom.current().nextDouble() * (Length-sublength));
        ...
        for (int i = 0; i < mutants; i++) {
            ...
            int[] parent = matingPool.get((int)(ThreadLocalRandom.current().nextDouble() * (matingPool.size()))).clone();
            ...
            //System.out.println();
                ...
                for (int x = 0; x<parent.length; x++) {
                    double mutate = ThreadLocalRandom.current().nextDouble();
                    if (mutate < mutationChance) {
                        int pos1 = (int)(ThreadLocalRandom.current().nextDouble() * (parent.length));
                        int pos2; 
                        do {
                            pos2 = (int)(ThreadLocalRandom.current().nextDouble() * (parent.length));
                        } while (pos2 == pos1);
...
//    } catch (Exception e) {
//        System.out.println("Caught Interrupted except.");
//    }
...
timer.scheduleAtFixedRate(new TimerTask() {
  ...
  public void run() {
    if (!repeat) {
        ...
        do {
                int rand = (int)(randomNum.nextDouble() * (colDest.size()));
    ...
    public static int[] generateRoute() {//int row, int col, int destinations, ArrayList<Integer> colDest, int[] route) {//Method to generate route
        ...
        int visit = 0;
                    //route[0] = 0;
                do {
                    int rand = (int)(ThreadLocalRandom.current().nextDouble() * (destinations.size()));
    ...
    public static void evaluate(ArrayList<int[]> generation, int[] allFit, float fitSum) {
        //int fitSum =0;

    //int min = Collections.min(List.allFit);
    }

    public static void select(ArrayList<int[]> generation, ArrayList<int[]> matingPool, int[] allFit, float fitSum, int min) {//Method to select individuals for mating pool (Roulette)

    }

    public static void crossover(ArrayList<int[]> generation, ArrayList<int[]> matingPool) {
